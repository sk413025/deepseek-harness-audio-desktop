...
- **0.4.12** (release REQUEST_VAD_BARGEIN_PAIRING; mock-tested with the real server rule): on `omni-duplex`, `live/open` and `session-params` refuse unpaired turn settings with 400 `INVALID_PARAM_COMBINATION` (key `overlapPolicy`) before any server connection: `overlapPolicy: barge_in_on_speech` needs `turnDetection: server_vad`, and `listen_only` cannot be combined with `server_vad` (the vLLM-Omni duplex server rejects both). `server_vad` alone still pairs `barge_in_on_speech`. The open reply, `live.state ready` and the close summary carry `turn {mode: native-duplex | server-vad | no-turn-detection, turnDetection, overlapPolicy, sources, nativeDuplexRequested, implementationLevel}`; native duplex is never labelled VAD. `GET capabilities` omni-duplex models carry `turnModes {default, modes[{mode, send}], rules}` so a UI offers one mode choice. `live.decision` reads the listen reason, `model_listen` and `model_turn_id` from `response.metadata`. Contract: `parallel-work/streaming/TASK_CONTRACT.md` §K.18.
- **0.4.11** (release DEMO-MINICPM-3 pre17 findings; mock-tested with vLLM-Omni 58adeec shapes): `live/close` ends every active response stream (`audio.end {status: "cancelled"}`) before waiting for the server's `session.closed` (up to 2 s, unchanged), so players can drop scheduled audio at close time; the close summary adds `close {requestedAt, streamsStoppedMs, serverClosed, serverCloseWaitMs}`. Server decisions are recorded instead of dropped: feed `live.decision` for `response.listen` (reason, modelListen, buffering), `response.speak` and `overlap.decision` (action, policy, reason, overlapMs); `live/close` `serverEvents {counts, lastAt, decisions, modelSilenceWindows, recentDecisions}` where a model-silence window is ≥ 3 s without model events (response.*, overlap.decision, speech, transcription) while input frames were forwarded. No timeout or turn policy changed. Contract: `parallel-work/streaming/TASK_CONTRACT.md` §K.17.
