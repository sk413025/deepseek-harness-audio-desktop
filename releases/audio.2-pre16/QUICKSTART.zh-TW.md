# DeepSeek Harness Audio（Local Build）· 兩模型 Demo 快速上手（audio.2-pre16）

- **版本：** `0.1.5-rc.1-audio.2-pre16`，macOS Apple Silicon（arm64）。
- **性質：** 依官方 DeepSeek Harness 0.1.5-rc.1 原始碼本機建置的**自訂版**，不是官方發行版。
- **簽章：** ad-hoc，**沒有** Developer ID 簽章、**沒有**公證。

**內建外掛（也可在 `plugins/` 單獨安裝）：**
- `dsh-dgx-audio` 0.4.10：vLLM／vLLM-Omni 音訊轉接（串流證據、Live 全雙工標示與 playback ack 修正）
- `dsh-voice-capture` 0.3.5：錄音、Live、邊收邊播與實際播放時間線、Stop 立即停止
...
| MiniCPM-o 4.5 Live 全雙工 | 前一版（pre13）真 USB 麥克風 Live 輸入與同時串流輸出已看到；Interrupt **未執行**；本版待驗 |
| MiMo-Audio-7B-Instruct | 前一版（pre13）真麥克風傳輸成功但語音串流 **FAIL**（近靜音輸入、僅 0.16 s 音訊、final-only）；本版邊收邊播與真麥克風**待驗** |
| 第二台實體 Mac | **未驗證** |
