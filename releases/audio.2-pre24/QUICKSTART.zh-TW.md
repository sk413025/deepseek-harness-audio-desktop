# DeepSeek Harness Audio（Local Build）· 兩模型 Demo 快速上手（audio.2-pre24）

- **版本：** `0.1.5-rc.1-audio.2-pre24`，macOS Apple Silicon（arm64）。App 名稱：**DeepSeek Harness Audio pre24 (Local Build)**。
...
- `dsh-audio-release-kit` 0.2.4：Audio servers 設定（DGX Spark 預設模型含正確請求設定：MiMo 回覆上限 200 tokens 與簡短系統提示、MiniCPM 關閉 thinking；已在 Audio models 設定 DGX 時，提示改用 Audio models）、DGX audio 模式
...
## 1. 安裝
1. 打開 dmg，把 **DeepSeek Harness Audio pre24 (Local Build)** 拖到「應用程式」。
