# DeepSeek Harness Audio（Local Build）· 兩模型 Demo 快速上手（audio.2-pre23）

- **版本：** `0.1.5-rc.1-audio.2-pre23`，macOS Apple Silicon（arm64）。App 名稱：**DeepSeek Harness Audio pre23 (Local Build)**。
...
- `dsh-audio-release-kit` 0.2.3：Audio servers 設定（**已預填 DGX Spark 位址與模型**）、DGX audio 模式
...
| **在 Harness 切換 DGX 模型** | audio.2-pre22 真機 PASS（同一組外掛；本版只改預設值檔的控制器指令與語音檔路徑寫法，並重新確認 Refresh 可連上控制器）：Audio models 按 Activate，按下到 Ready：MiniCPM → MiMo 4 分 49 秒；MiMo → MiniCPM 約 3 分 47 秒 |
...
## 1. 安裝
1. 打開 dmg，把 **DeepSeek Harness Audio pre23 (Local Build)** 拖到「應用程式」。
...
## 2. 使用
1. 第一次開啟若出現「Add an API key to get started」，按 **Configure later**（音訊模型不需要 DeepSeek API key）。
